...
const RegisterForm = ({ onRegistered }: RegisterFormProps) => {
  ...
  const tryRegister = () => {
    ...
    if (!validateHospitalEmail(email)) {
      alert(
        "Please use a professional hospital email (hospital/health/clinic or .edu/.org)",
      );
      return;
    }
    ...
    return (
      <div className="p-4 bg-accent/10 rounded-md text-center space-y-3">
        ...
        <p className="text-sm text-muted-foreground">
          Your hospital registration request has been submitted to the admin for
          verification.
        </p>
        <p className="text-sm text-accent font-medium">
          Once approved, you'll be able to login with your registration ID and
          password.
        </p>
        <Button
          variant="outline"
...
const HospitalPortal = () => {
  ...
  const [view, setView] = useState<"dashboard" | "submit" | "my" | "profile">(
    "dashboard",
  );
  ...
  useEffect(() => {
    ...
    if (session?.userType === "hospital" && session?.userId) {
      setLoggedIn(true);
      setRegId(session.userId);
    }
  }, []);
          ...
          <div className="bg-card rounded-xl border border-border p-6 shadow-sm">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  Sign in with your Hospital ID or register a new account
                </p>
              </div>
            </div>
                ...
                <div className="space-y-3">
                  ...
                  <Button
                    ...
                    onClick={() => {
                      ...
                      if (!regId || !pwd) {
                        alert("Please enter registration ID and password");
                        return;
                      }
    ...
    <div className="flex min-h-screen">
      ...
      <nav className="w-64 bg-slate-50 border-r border-border p-6">
        ...
        <ul className="space-y-3 text-sm">
          ...
          <li>
            <button
              className={`w-full text-left ${view === "submit" ? "font-semibold text-accent" : ""}`}
              onClick={() => navigate("/hospital/submit")}
            >
              Submit Patient
            </button>
          </li>
          <li>
            <button
              className={`w-full text-left ${view === "my" ? "font-semibold text-accent" : ""}`}
              onClick={() => navigate("/hospital/my")}
            >
              My Submissions
            </button>
          </li>
      ...
      <main className="flex-1 p-8">
        ...
        {view === "dashboard" &&
          (() => {
            const myCases = hospitalSubmissions.filter(
              (s: any) => s.regId === (regId || "UNREGISTERED"),
            );
                  ...
                  {recentCases.length > 0 ?
                    <ul className="space-y-3">
                      {recentCases.map((s: any, i: number) => (
                        <li
                          key={i}
                          className="p-4 bg-muted/10 rounded-md flex justify-between items-center"
                        >
                          <div>
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  if (!/^[0-9]{4}$/.test(aadhaarLast4)) {
                    alert("Aadhaar last 4 digits must be 4 numbers");
                    return;
                  }
                  if (!diseaseName.trim()) {
                    alert("Disease name required");
                    return;
                  }
                  ...
                  if (Number.isNaN(amt) || amt <= 0) {
                    alert("Amount required must be a positive number");
                    return;
                  }
                  if (!doctorRegNo.trim()) {
                    alert("Doctor registration number required");
                    return;
                  }
                  if (insuranceAvailable) {
                    const cov = Number(insuranceCoverage);
                    if (Number.isNaN(cov) || cov < 0 || cov > 100) {
                      alert("Insurance coverage must be 0-100");
                      return;
                    }
                  }
                  // submit
                  const ok = submitPatientCase(regId || "UNREGISTERED", {
                    ...
                    insuranceAvailable,
                    insuranceCoverage:
                      insuranceAvailable ? Number(insuranceCoverage) : 0,
                    prescriptionFile,
                    medicalReportFile,
                  });
                  if (ok) {
        ...
        {view === "my" && (
          <div className="bg-card rounded-xl border border-border overflow-hidden">
            <table className="w-full text-sm">
              ...
              <tbody>
                {hospitalSubmissions
                  .filter((s) => s.regId === (regId || "UNREGISTERED"))
                  .map((s, i) => (
                    <tr key={i} className="border-b border-border">
                      ...
                      <td className="px-5 py-3 font-semibold">
                        ₹
                        {s.amountRequired?.toLocaleString?.() ||
                          s.estimatedCost}
                      </td>
